    ...
    async with connect(websocket_endpoint) as ws:
        while True:
            ...
            if message_type == "session_welcome":
                ...
                print(f"Connected to websocket session @{websocket_endpoint}, Status: {session_status}")
                print(f"session ID: {session_id}")

                subscriptions.stream_online(url=subscription_endpoint, token= token, client_id= client_id, session_id= session_id)
